...
num_av_0_5=0;num_av_5_0=0;num_av_5_10=0;num_av_10_5=0;num_av_10_up=0;num_av_10_dn=0;rpa_dist=0;
for i=1:cyc_tim
...
%distance
for i=1:cyc_tim-1
    dis(i)=0.5*((cyc(i,2)*10/36)+(cyc(i+1,2)*10/36));% trapezoidal formula for integral (m)
    dist=dist+dis(i);
end
%------------------------------------------------------------------------
% acceleration
...
for i1=1:cyc_tim-1
    if a(i1,2)>0
        rpa_dist=rpa_dist+0.5*((cyc(i1,2)*10/36)+(cyc(i1+1,2)*10/36));% trapezoidal formula for integral (m)
    end
end
...
for i=2:cyc_tim
    ...
    if a(i,2)>0
        ...
        pke_dis=pke_dis+0.5*((cyc(i,2)*10/36)+(cyc(i+1,2)*10/36));% trapezoidal formula for integral (m)
    end
end
