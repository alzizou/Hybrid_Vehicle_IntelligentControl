...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_conv_906_eng_hydr_new_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_conv_906_eng_hydr_new_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_conv_906_eng_hydr_new_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "conv_906_eng_hydr_new") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_conv_906_eng_hydr_new_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_conv_906_eng_hydr_new_get_eml_resolved_functions_info
  ...
  if (strcmp(machineName, "conv_906_eng_hydr_new") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_conv_906_eng_hydr_new_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_conv_906_eng_hydr_new_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"conv_906_eng_hydr_new")) {
    return(sf_conv_906_eng_hydr_new_method_dispatcher(simstructPtr,
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    conv_906_eng_hydr_new_terminator();
    sf_debug_terminate();
  }
...
extern void conv_906_eng_hydr_new_register_exported_symbols(SimStruct* S);
extern void conv_906_eng_hydr_new_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  conv_906_eng_hydr_new_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      conv_906_eng_hydr_new_debug_initialize();
    }

    conv_906_eng_hydr_new_initializer();
  }
