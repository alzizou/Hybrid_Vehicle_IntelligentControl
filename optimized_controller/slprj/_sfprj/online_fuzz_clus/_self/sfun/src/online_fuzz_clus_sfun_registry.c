...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_online_fuzz_clus_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_online_fuzz_clus_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_online_fuzz_clus_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "online_fuzz_clus") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_online_fuzz_clus_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_online_fuzz_clus_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "online_fuzz_clus") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_online_fuzz_clus_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_online_fuzz_clus_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"online_fuzz_clus")) {
    return(sf_online_fuzz_clus_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void online_fuzz_clus_register_exported_symbols(SimStruct* S);
extern void online_fuzz_clus_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  online_fuzz_clus_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      online_fuzz_clus_debug_initialize();
    }

    online_fuzz_clus_initializer();
  }
