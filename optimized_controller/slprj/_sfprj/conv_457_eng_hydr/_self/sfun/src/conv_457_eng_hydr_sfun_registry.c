...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_conv_457_eng_hydr_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_conv_457_eng_hydr_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_conv_457_eng_hydr_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "conv_457_eng_hydr") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_conv_457_eng_hydr_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_conv_457_eng_hydr_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "conv_457_eng_hydr") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_conv_457_eng_hydr_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_conv_457_eng_hydr_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"conv_457_eng_hydr")) {
    return(sf_conv_457_eng_hydr_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void conv_457_eng_hydr_terminator(void);
...
extern void conv_457_eng_hydr_initializer(void);
extern void conv_457_eng_hydr_register_exported_symbols(SimStruct* S);
extern void conv_457_eng_hydr_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  conv_457_eng_hydr_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      conv_457_eng_hydr_debug_initialize();
    }

    conv_457_eng_hydr_initializer();
  }
