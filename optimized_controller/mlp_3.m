...
% close all
% load('fun_2_data.mat');
function out=mlp_3(inputs,outputs)
% avg_err_tot=0;
% avg_err=zeros(1,216);
...
dat_vld_inp=zeros(num_vld,3);dat_vld_otp=zeros(num_vld,1);
%-------------------------------------------------------------------------
...
input_data=[dat_trn_inp(:,1)' dat_vld_inp(:,1)' dat_tst_inp(:,1)';dat_trn_inp(:,2)' dat_vld_inp(:,2)' dat_tst_inp(:,2)';dat_trn_inp(:,3)' dat_vld_inp(:,3)' dat_tst_inp(:,3)'];
output_data=[dat_trn_otp' dat_vld_otp' dat_tst_otp'];
% m=50;% number of neurons in the seconed layer
epoch_num=5000;% maximum number of epochs
net_ali=newff(input_data,output_data,[20 20],{'logsig','logsig','purelin'},'trainlm','learngd');
